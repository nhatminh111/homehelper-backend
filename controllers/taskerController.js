...
exports.updateAddress = async (req, res) => {
  try {
    ...
    if (!addressToUpdate || addressToUpdate.user_id !== user_id) {
      return res.status(404).json({ message: 'Địa chỉ không tồn tại hoặc không được phép' });
    }
    ...
    console.log('📊 Search API response:', searchResponse.data);
    if (!searchResponse.data || searchResponse.data.length === 0) {
      console.warn('⚠️ Không tìm thấy kết quả, cập nhật địa chỉ mà không có tọa độ');
      const updatedAddress = await Address.update(address_id, trimmedAddress, 0, 0);
      return res.json({ ...updatedAddress, message: 'Đã cập nhật địa chỉ nhưng không tìm thấy tọa độ trên bản đồ. Vui lòng kiểm tra lại định dạng.' });
    }
    ...
    if (!exactMatch) {
      console.warn('⚠️ Không tìm thấy kết quả chính xác, cập nhật địa chỉ mà không có tọa độ');
      const updatedAddress = await Address.update(address_id, trimmedAddress, 0, 0);
      return res.json({ ...updatedAddress, message: 'Đã cập nhật địa chỉ nhưng không tìm thấy kết quả chính xác. Vui lòng kiểm tra lại.' });
    }
    ...
    if (!refId) {
      console.warn('⚠️ Không tìm thấy ref_id, cập nhật địa chỉ mà không có tọa độ');
      const updatedAddress = await Address.update(address_id, trimmedAddress, 0, 0);
      return res.json({ ...updatedAddress, message: 'Đã cập nhật địa chỉ nhưng không tìm thấy ref_id. Vui lòng kiểm tra lại.' });
    }
    ...
    if (!lat || !lng || lat === 0 || lng === 0) {
      ...
      const updatedAddress = await Address.update(address_id, trimmedAddress, 0, 0);
      return res.json({ ...updatedAddress, message: 'Đã cập nhật địa chỉ nhưng không lấy được tọa độ hợp lệ. Vui lòng kiểm tra lại.' });
    }

    // Cập nhật DB
    const updatedAddress = await Address.update(address_id, trimmedAddress, lat, lng);
...
exports.getAddressesByUserId = async (req, res) => {
  try {
    ...
    if (!addresses || addresses.length === 0) {
      return res.status(200).json({ message: 'Không tìm thấy địa chỉ', addresses: [] });
    }
    res.json({ addresses });
  } catch (error) {
...
exports.searchNearbyUsers = async (req, res) => {
  try {
    ...
    // Validate input
    if (!lat || !lng || !radius) {
      return res.status(400).json({ success: false, message: 'Yêu cầu cung cấp lat, lng và radius' });
    }

    if (typeof radius !== 'number' || radius < 1000 || radius > 15000) {
      return res.status(400).json({ success: false, message: 'Bán kính phải là số từ 1000 đến 15000 mét' });
    }
    ...
    if (min_rating !== null) {
      ...
      if (isNaN(ratingNum) || ratingNum < 1 || ratingNum > 5) {
        return res.status(400).json({ success: false, message: 'min_rating phải là số từ 1 đến 5' });
      }
    }

    // Validate services
    if (services.length > 0 && (!Array.isArray(services) || !services.every(s => Number.isInteger(s) && s > 0))) {
      return res.status(400).json({ success: false, message: 'services phải là array các service_id số nguyên dương' });
    }

    // Log input parameters for debugging
    console.log(`🔍 Tìm kiếm tasker gần: lat=${lat}, lng=${lng}, radius=${radius}, services=${JSON.stringify(services)}, min_rating=${min_rating}`);

    // Get filtered tasker addresses with their service variants
    const allAddresses = await Address.findFilteredTaskerAddresses(min_rating, services);

    if (allAddresses.length === 0) {
      console.log('⚠️ Không tìm thấy tasker phù hợp với bộ lọc.');
      return res.json({ success: true, users: [], total_filtered_before_geofence: 0, total_in_range: 0 });
    }
    ...
    const usersInRange = allAddresses
      .map(addr => {
        ...
        const a = Math.sin(dLat / 2) * Math.sin(dLat / 2) +
                  Math.cos(toRad(lat)) * Math.cos(toRad(addr.lat)) * 
    ...
    console.log(`✅ Tìm thấy ${usersInRange.length} tasker trong bán kính ${radius}m. Tổng tasker trước khi lọc khoảng cách: ${allAddresses.length}`);
